...
def merge(first: Predicate, second: Predicate):
    ...
    assert second.merge(first) == result
    return result
...
def test_binary_eq_conflict():
    ...
    assert merge(Predicate.binary('==', 3), Predicate.binary('>=', 3)) == Predicate.binary('==', 3)

    assert merge(Predicate.binary('==', 3), Predicate.binary('>', 2)) == Predicate.binary('==', 3)
